...
describes what users get. Pick a format with `-o`, or let topica choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
topica <command> -o table   # aligned columns for reading
topica <command> -o jsonl   # one JSON object per line, for piping
topica <command> -o json    # a single JSON array
topica <command> -o csv     # spreadsheet friendly
topica <command> -o tsv     # tab-separated
topica <command> -o url     # just the URL column
topica <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
topica <command> --fields id,title,url
```
...
```bash
topica <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
topica <command>            # a table, because this is a terminal
topica <command> | wc -l    # JSONL, because this is a pipe
```
